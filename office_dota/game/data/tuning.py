...
ECONOMY: dict = {
    ...
    "dynamic_applies_to": {
        ...
        "hero_kill_gold": True,
        "hero_kill_xp": True,
        "building_gold": False,          # объекты — награда за игру, не за гандикап
        "passive_gold": False,
    },
}
...
4. ПРОВЕРКА ДЛИНЫ МАТЧА (иначе всё выше — бумага)
   ...
   первая выигранная драка 5-на-5 после 11:00 конвертируется в T3 + бараки + трон
...
5. ЧТО Я МЕНЯЛ, ЧТОБЫ ПОПАСТЬ В КОРИДОР
   Первый заход: интервал волны 20 с и опыт крипов как в доте (62/76/88).
   Получилось 18-й уровень на 8:40 и T4 на 11:30 — матч схлопывался в 10-12 минут,
   ульта 3-го уровня приходила раньше, чем команда успевала собраться на пуш.
   Поднял интервал волны до 25 с и снизил опыт крипов до 55/70/85 (-11%).
   Второй заход: пассив 3.5 B/с давал T3 на 7:30 — предметы обгоняли уровни,
   драки превращались в «кто первый купил». Пассив опущен до 2.5 B/с.
